...
const root = ref(null)
const { state, playlist, select } = useAudioPlayer()
let ctx

onMounted(() => {
  ctx = gsap.context(() => {
    revealUp('.playlist-head, .playlist-card', { trigger: root.value })
  }, root.value)
})
...
<template>
  <section ref="root" class="px-6 py-20 md:py-28">
    <div class="playlist-head mx-auto mb-10 max-w-xl text-center">
      <h2 class="script-title text-6xl md:text-7xl">Our Soundtrack</h2>
      <p class="mt-3 text-inksoft">lagu-lagu yang rasanya kayak kita 🎧</p>
    </div>

    <div class="playlist-card soft-card mx-auto max-w-xl overflow-hidden">
      <button
